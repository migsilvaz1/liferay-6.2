...
<%
String redirect = ParamUtil.getString(renderRequest, "backURL");
if(redirect != ""){
%>
<div class="w95 center chartBack" >
	<liferay-ui:header
	    backURL = "<%=redirect %>"
	    title=''
	/>
...
<%
String search2 = ParamUtil.getString(renderRequest, "searchPublisher");
if(search2 != ""){
	renderRequest.setAttribute("searchPublisher", search2);
	request.setAttribute("searchPublisher", search2);
}
if(enableSearchBox){
%>
<portlet:renderURL var="searchPublisherURL"><portlet:param name="backURL" value="<%=themeDisplay.getURLCurrent()%>" /></portlet:renderURL>
<div class="container-fluid">
	<div class="row-fluid span12" id="search">
<form action="<%= searchPublisherURL %>" class="form-inline span12" method="post">
	  <div class="input-append pull-left span5">
	    <input id="<portlet:namespace/>searchPublisher" name="<portlet:namespace/>searchPublisher" type="text" placeholder='<%=LanguageUtil.get(locale, "hook.search")%>' value='<%=ParamUtil.getString(renderRequest, "searchPublisher")%>'>
...
portletURL.setParameter("searchPublisher", search2);
...
<%
if(totalGeneral != 0){
searchContainer.setTotal(totalGeneral);
}
%>
