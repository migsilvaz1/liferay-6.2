...
<%@page import="com.liferay.portal.service.PortletPreferencesServiceUtil"%>
<%@ include file="/html/portlet/document_library_display/init.jsp" %>
<!-- ------------------------------------------------------------------------------------------------ -->
<%
	String carpetaCiudadano = portletPreferences.getValue("carpetaDelCiudadano", "false");
	Boolean contenedoresContraibles;
	if(carpetaCiudadano.trim().toLowerCase().compareTo("true") == 0){
		//Hacemos los contenedores no contraibles 			
		contenedoresContraibles = Boolean.FALSE;
	}else{
		contenedoresContraibles = Boolean.TRUE;
	}
%>
<!-- ------------------------------------------------------------------------------------------------ -->
<%
	...
	<c:when test='<%= topLink.equals("home") %>'>
		<c:if test="<%= (folder != null) %>">
			<liferay-ui:header
				backURL="<%= redirect %>"
				localizeTitle="<%= false %>"
				title='<%=	carpetaCiudadano.compareTo("true") == 0 && folder.getName().compareTo(user.getScreenName()) == 0? LanguageUtil.get(locale, "misDocumentos.portlet.label.raiz") : folder.getName() %>'
			/>
		</c:if>
		<aui:row>
			<aui:col cssClass="lfr-asset-column lfr-asset-column-details" width="<%= showFolderMenu ? 75 : 100 %>">
				<liferay-ui:panel-container extended="<%= false %>" id='<%= renderResponse.getNamespace() + "documentLibraryDisplayInfoPanelContainer" %>' persistState="<%= true %>">
					...
					<c:if test="<%= foldersCount > 0 %>">
						<liferay-ui:panel collapsible="<%=contenedoresContraibles%>" cssClass="view-folders" extended="<%=contenedoresContraibles%>" id='<%= renderResponse.getNamespace() + "documentLibraryDisplayFoldersListingPanel" %>' persistState="<%= true %>" title='<%= (folder != null) ? "subfolders" : "folders" %>'>
					...
					<liferay-ui:panel collapsible="<%=contenedoresContraibles%>" extended="<%=contenedoresContraibles%>" id='<%= renderResponse.getNamespace() + "documentLibraryDisplayDocumentsListingPanel" %>' persistState="<%= true %>" title="documents">
						<%@ include file="/html/portlet/document_library/view_file_entries.jspf" %>
					</liferay-ui:panel>
			...
			<c:if test="<%= showFolderMenu %>">
				<aui:col cssClass="lfr-asset-column lfr-asset-column-actions" last="<%= true %>" width="<%= 25 %>">
					<div class="lfr-asset-summary">
						...
						/>
						<%
							String nombreFicheroOpciones;
							String nombrePorDefecto = (folder != null) ? HtmlUtil.escape(folder.getName()) : LanguageUtil.get(pageContext, "home");
							if(carpetaCiudadano.compareTo("true") == 0 && nombrePorDefecto.compareTo(user.getScreenName()) == 0){
								nombreFicheroOpciones = LanguageUtil.get(locale, "misDocumentos.portlet.label.raiz"); 
							}else{
								nombreFicheroOpciones = nombrePorDefecto;
							}
						%>
						<div class="lfr-asset-name">
							<h4><%=nombreFicheroOpciones%></h4>
						</div>
					</div>
