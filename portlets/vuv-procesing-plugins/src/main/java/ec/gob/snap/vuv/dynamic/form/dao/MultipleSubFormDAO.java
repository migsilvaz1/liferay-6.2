...
public class MultipleSubFormDAO {
	...
	public MultipleSubFormDAO(MultipleSubForm tableData, Locale locale)
		throws ParseException, TransformerConfigurationException {
		...
		AddItemText itemADD = tableData.getAddItemText();
		addItem = new LabelDAO((AddItemText) itemADD);

		CancelItemText intemCancel = tableData.getCancelItemText();
		cancelItem = new LabelDAO((CancelItemText) intemCancel);

		NewItemText itemNew = tableData.getNewItemText();
		saveItem = new LabelDAO((NewItemText) itemNew);

		List<MultipleSubForm.SubForm> subFormRows = new ArrayList<MultipleSubForm.SubForm>();
		subFormRows.addAll(tableData.getSubForm());
		for (SubForm subForm : subFormRows) {
			List<JAXBElement<?>> fields = subForm.getOutputTextOrInputTextOrInputDate();
	...
	private JSONArray tableFieldsJSON(List<JAXBElement<?>> columns, Locale locale)
		throws TransformerConfigurationException, ParseException {
			...
			if (element.getValue() instanceof InputLabel) {
				...
				if (inputText.getType().equalsIgnoreCase(String.class.getName())) {
					if (inputText.getHeight() == 0) {
						...
						jsonField.put("type", "textAreaInput");
					}
				}
				...
				jsonField.put("type", "inputDate");
				jsonField.put("id", inputText.getInputDateID());
				jsonField.put("key", inputText.getInputDateName());
				jsonField.put("name", inputText.getLabeli18n(locale));
				jsonField.put("value", inputText.getValue());
				jsonField.put("required", inputText.isFieldRequired());
