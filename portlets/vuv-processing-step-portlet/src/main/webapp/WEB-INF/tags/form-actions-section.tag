...
<%@tag import="ec.gob.snap.vuv.dynamic.form.entities.Template.Formulario.Acciones"%>
...
<%@ attribute name="actions" required="true" rtexprvalue="true" type="java.util.List" %>
<portlet:defineObjects />
	<div class="alert alert-buttons">
		...
		%>
		<aui:button 
			id="<%=buttonDAO.getInputID() %>"
			name="<%=buttonDAO.getInputName() %>"
			disabled="<%=buttonDAO.isReadOnly()%>"
			action="<%=buttonDAO.getAction()%>" type="submit"
			value="<%=buttonDAO.getLabeli18n(request.getLocale())%>"></aui:button>
		<%
			}
				}
		%>
